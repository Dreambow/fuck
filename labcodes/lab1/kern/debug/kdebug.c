...
print_stackframe(void) {
      ...
      */
    uint32_t caller = read_ebp();
    uint32_t pc = read_eip();


    for (int i = 0; i < STACKFRAME_DEPTH; i++) {
	if(caller == 0) break;
        cprintf("ebp:0x%08x eip:0x%08x args:", caller, pc);
        uint32_t *args = (uint32_t *)caller + 2;
        for (int j = 0; j < 4; j++) {
            cprintf("0x%08x ", args[j]);
        }
        cprintf("\n");
        print_debuginfo(pc - 1);
        pc = ((uint32_t *)caller)[1];
        caller = ((uint32_t *)caller)[0];
    }
}
